...
{
    if (((c[0] >= 'A' && c[0] <= 'J') || (c[0] >= 'a' && c[0] <= 'j')) && ((c[1] >= '1' && c[1] <= '9' && c[2] == '\0') || (c[1] == '1' && c[2] == '0')))
    {
        return true;
    }
    else
...
bool HitOrMiss(struct Player *p, char coordinates[], char difficultyLevel[])
{
    if (validCoordinates(coordinates))
    {
        ...
        if (p->grid[row][col] == '~')
        {
            ...
            {
                p->grid[row][col] = 'o'; // changes display on grid if missed and if in easy mode
            }
            return false;
        }
...
void main(void)
{
    ...
    while ((strcmp(difficultyLevel, "easy") != 0 && strcmp(difficultyLevel, "hard") != 0))
    { // keeps on asking until input is valid
    ...
    while (!p1.placedShips || !p2.placedShips)
    { // asks both players starting with the chosen one for the placement of ships
